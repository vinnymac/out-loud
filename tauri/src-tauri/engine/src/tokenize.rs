//! Phoneme → token-id mapping. Ported verbatim from the Node engine's vocab
//! (tauri/sidecar/tts-worker.ts). Unknown characters fall back to 16 (space).

/// Map a single IPA character to its Kokoro token id.
pub fn token_id(c: char) -> i64 {
    match c {
        ';' => 1, ':' => 2, ',' => 3, '.' => 4, '!' => 5, '?' => 6,
        '—' => 9, '…' => 10, '"' => 11, '(' => 12, ')' => 13,
...
/// Tokenize an IPA string by Unicode scalar value (matches the JS code-point
/// iteration). Unknown characters → 16.
pub fn tokenize(phonemes: &str) -> Vec<i64> {
    phonemes.chars().map(token_id).collect()
}
...
mod tests {
    ...
    #[test]
    fn known_mappings() {
        assert_eq!(tokenize("ðɪs"), vec![81, 102, 61]);
        assert_eq!(tokenize("ɪz"), vec![102, 68]);
        // unknown char (e.g. the digit '2' used in cmn tones) → fallback 16
        assert_eq!(token_id('2'), 16);
        assert_eq!(token_id('ˈ'), 156);
    }
}
